 ...
 */
function buildValidatorFromType(order: OrderTypeOption): Validator {
    let compare = ([a, b]: string[]) => {
        const l = a.length > b.length ? a.length : b.length
        for (let i = 0; i < l; i++) {
            if (a[i - 1] === b[i - 1] && a[i] !== b[i]) {
                return b[i] === undefined || a[i] < b[i]
            }
        }
        return true
    }
        ...
        return [
            {
                ...
                ignore: () => false,
                isValidOrder: buildValidatorFromType(type),
                minKeys,
                orderText: `${type}ending`,
            },
        ]
    }
...
export default createRule("sort-keys-lokalise", {
    meta: {
        docs: {
